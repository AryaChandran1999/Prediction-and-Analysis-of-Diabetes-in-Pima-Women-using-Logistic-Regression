    ...
    tabItems(
      ...
      tabItem(tabName = "home",
              ...
              fluidRow(
                ...
                column(width=12,tags$p(align = "justify",width=12,HTML("The squad of 6 worked together to brainstorm and de-coded many codes. 
                                                    As the name suggests “THE OUTLIERS”, all of us having different backgrounds 
                                                    helped us in exchanged ideas.")))
              ...
              fluidRow(column(12,
                      box(title="Predictor Variables",solidHeader = TRUE,status = "primary",width = '1100px',
                          ...
                          column(width=12,tags$h4(width=8,HTML("It’s evident that women who have been diagnosed with Diabetes have had more pregnancies 
                                                  than women who were not (in this dataset). However, from the histogram, there’s no clear relationship
...
tabItem(tabName = "predict",
      ...
      column(12,box(title="Outcome",width = 1000,height=400,textOutput(outputId = "outcome"))))
),
...
server <- function(input, output) {
  ...
  p2 <- ggplot(Datafinal,aes(x = Pregnancies,fill = factor(Outcome))) + 
  ...
  # gridExtra::grid.arrange(p9, ncol = 2)})
  # 
  # output$in2_graph <- renderPlot({
  p10 <- ggplot(Datafinal, aes(Insulin, fill = factor(Outcome))) +
    ylim(0,30) +
  ...
  # output$dpf2_graph <- renderPlot({
    p13 <- ggplot(Datafinal, aes(DiabetesPedigreeFunction,fill = factor(Outcome))) +
      geom_histogram() +
      theme(legend.position = "bottom",plot.title = element_text(color="black", size=16, face="bold.italic",hjust=0.5)) +
  ...
  output$age1_graph <- renderPlot({
    p15 <- ggplot(Datafinal, aes(Age, fill = factor(Outcome))) +
      geom_histogram(binwidth = 5) +
      theme(legend.position = "bottom",plot.title = element_text(color="black", size=16, face="bold.italic",hjust=0.5)) +
      ggtitle("Variation of Age of women Vs Diabetes")
  #   gridExtra::grid.arrange(p15, ncol = 2)}) 
  #   
  # output$age2_graph <- renderPlot({
